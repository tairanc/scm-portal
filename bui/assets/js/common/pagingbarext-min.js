/**
 * 基于BUI分页插件增加 每页显示条数功能
 *
 * 使用说明：分页插件需要渲染所以需要装载分页控件容器
 *
 * 使用  html 页面需要定义装载容器  如 ：<div id="bar" class="bui-grid"></div>
 *       js 代码渲染：
 *                  BUI.use(['common/pagingbarext'],function(PagingBarExt){
 *
 *                              new PagingBarExt({
 *                                  render: "#bar",                     //指定容器id
 *                                  elCls: 'bui-pagingbar bui-bar',     //样式
 *                                  store: store                        //加载数据
 *                                  pagingBarArr: [10,20,50,100,200]    //select option选项 页面条数  ，可选参数
 *                                  jumpToPageCallBack: function        //分页跳转后执行函数 ，可选参数
 *                              }).render();
 *                      })
 *
 ...
 * @ignore
 */
define('common/pagingbarext', ['bui/toolbar/bar'], function (require) {
     ...
     */
    var PagingBarExt = Bar.extend([Bindable],
        {
            /**
            ...
            jumpToPage: function (page) {
                ...
                if(_self.get("jumpToPageCallBack"))_self.get("jumpToPageCallBack")();
            },
            jumpToPageFix: function(page,formData){
                if (page <= 0 || page > this.get('totalPage')) {
                    return;
                }
                var _self = this,
            ...
            _getPageSelect: function () {
                var _self = this,
                    render = _self.get('render'),
                    pagingBarArr = _self.get("pagingBarArr");

                var opts = "";
                for (var i = 0; i < pagingBarArr.length; i++) {
                    opts += '<option value="' + pagingBarArr[i] + '">' + pagingBarArr[i] + '条/页</option>'
                }

                return {
                    content:
                    '<div>' +
                    '<select style="width: 80px;height:20px;margin: 0 4px;padding:0 4px; "id="' + render.replace("#", "") + 'Sel">' +
                    opts +
                    '</select>' +
                    '</div>',
                    disabled: true
            ...
            _bindButtonEvent: function () {
                ...
                $(_self.get('render') + "Sel").unbind("change").change(function () {
                    _self.set("pageSize", Number($(this).val()));
                    _self.get('store').set("pageSize",Number($(this).val()));
                    _self.jumpToPage(1);
                });
            ...
            _getItems: function () {
                ...
                items.push(_self._getPageSelect());
                //total page of store
