...
define('common/main',['bui/menu','bui/tab','bui/tree'],function(require) {
  //定义全局命名空间
  ...
  var CLS_SELECTE = 'dl-selected',//选中的模块样式
    ...
    function getVersion(href) {
        var version = "";
        if (localStorage.getItem("config")&&href) {
  ...
  function tabNav(moduleId, tabConfig, menuConfig, collapsed, homePage) {
    ...
    //     leafTpl : '<li class="menu-second"><div><a href="{href}"><em>{text}</em></a></div></li>',
    //     nodes : $.newGetShowMenu()
    // });
    
    var _self =this,
      //menu = new Menu.SideMenu(menuConfig), 
      menu=new Tree.TreeMenu({
            render : menuConfig.render,
            elCls : 'scm-side-menu',  
            itemCls : 'bui-menu-item',
            expandAnimate : true,
            showIcons : true,
            dirTpl : '<li class="menu-second"><div class="bui-menu-title"><span class="bui-menu-title-text">{text}</span></div></li>',
            leafTpl : '<li class="menu-second"><div><a href="{href}"><em>{text}</em></a></div></li>',
            nodes : $.newGetShowMenu()
        });
    ...
    menu.on('itemclick', function (ev) {
      ...
      if(item&&item.attr("href")){
        _self.tab.addTab({id: item.attr('id'), title: item.attr('text'), href: item.attr('href') + getVersion(item.attr('href')),closeable : item.attr('closeable')},true);
        
        //_self.tab.addTab({id: item.get('id'), title: item.get('text'), href: item.get('href') + getVersion(item.get('href')),closeable : item.get('closeable')},true);
      }
    });
    ...
    menu.on('itemselected',function(ev){
      var item = $(ev.item);
      if(item){
        setNavPosition(moduleId,item.attr('id'));
      }
    ...
    menu.render();
    //tree.render();
    $(menuConfig.render).bind("click",function(e){
      e.preventDefault();
    });
  }
  ...
  BUI.augment(mainPage,{
    ...
    openPage : function(pageInfo){
      ...
      if(module){
        ...
        if(menuItem){
          ...
          tab.addTab({id: id, title: title, href: href + getVersion(href), sourceId: sourceId,closeable: closeable},reload);
        }
    ...
    _setPageSelected:function(moduleIndex,pageId,isReload,search){
      ...
      if(module && pageId){
        ...
        var item = module.menu.getItem(pageId);//module.menu.getSelected(),
          tab = module.tab,
          href = '',
          suffixIndex = -1;
        ...
        if(item && item.id === pageId){          
          ...
          module.tab.addTab({id: item.id, title: item.text,closeable : item.closeable, href: href + getVersion(href)},!!isReload);
          ...
          tab.addTab({id:pageId,title:'',href:href + getVersion(href)},!!isReload);
        }
      }
    },
