...
$(function(){
    ...
    $.shipMentsTypeUpdateApp = {
        _isValid : "",
        init:function(){

            var id = $.getUrlParam("id")

            $.shipMentsTypeUpdateApp.fileForm(id);
        	...
        	BUI.use(['bui/form','bui/tooltip'],function(Form, Tooltip){
                ...
                $("#save_btn").on("click",function(){
                    ...
                    if(form.isValid()){
                        $.shipMentsTypeUpdateApp.save(formData, tip);
                    }
                });

                $("#btn_list").on("click",function(){
                    window.location.href = "shipMentsTypeList.html";
                });
        ...
        fileForm:function (id) {
            ...
            aContent.success = function(result,textStatus){
                if(result.appcode != 1){
                    ...
                    $.shipMentsTypeUpdateApp._isValid = dictTypeDtl.isValid;
    ...
    $(document).ready(function(e) {
        $.shipMentsTypeUpdateApp.init();
    });
    //页面完全加载完成后设置单选框默认选中
    window.onload=function(){
        $.setRadioChecked('isValid', $.shipMentsTypeUpdateApp._isValid);
    }
}(jQuery));
