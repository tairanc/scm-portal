...
$(function () {
    ...
    $.wmsUserAccreditInfoUpdateApp = {
        ...
        init: function () {
            ...
            $.wmsUserAccreditInfoUpdateApp.queryIsValid(id);
            ...
            BUI.use(['bui/form', 'bui/grid','bui/data','bui/tooltip'], function (Form, Grid,Data,Tooltip) {
                ...
                grid.on("aftershow", function() {
                    $.wmsUserAccreditInfoUpdateApp.fileForm(id);
                })
                grid.render();
                    ...
                    if (formData.userType != "overallUser" && formData.channelCode == "") {
                        ...
                        if (form.isValid()) {
                            if ($("input[name='roleNames']:checked").length == 0) {
                                ...
                                $.wmsUserAccreditInfoUpdateApp.updateUser(formData, tip);
                            }
                        }
                    }
                });
                ...
                $("#userType").on("change", function () {
                    ...
                    // var purchaseRole = $.wmsUserAccreditInfoUpdateApp.purchaseRole;
                    ...
                    switch (val) {
                        ...
                        case "mixtureUser" :
                            //混合用户
                            $("#channelTypeDiv").show();
                            $.wmsUserAccreditInfoUpdateApp.queryRoleList("",val);
                            break;
                        case  "overallUser" :
                            $("#channelTypeDiv").hide();
                            $.wmsUserAccreditInfoUpdateApp.queryRoleList("wholeJurisdiction",val);
                            //全局用户
                            break;
                        case "channelUser":
                            //渠道用户
                            $("#channelTypeDiv").show();
                            $.wmsUserAccreditInfoUpdateApp.queryRoleList("channelJurisdiction",val);
                            break;
                    }
                ...
                $("#btn_list").on("click", function () {
                    window.location.href = "wmsUserAccreditInfoList.html";
                    //window.history.go(-1);
                });
                ...
                //             //传到后台做校验
                //             $.wmsUserAccreditInfoUpdateApp.checkPurchase();
                //         }
                //     }
                // });
            });
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    if (roleType == "") {
                        ...
                        $.wmsUserAccreditInfoUpdateApp.forRole(allRole, "channelJurisdiction");
                        $("#selectRole").append("<div style='margin: 10px 0'>全局用户：</div>");
                        $.wmsUserAccreditInfoUpdateApp.forRole(allRole, "wholeJurisdiction");
                    } else {
                        $.wmsUserAccreditInfoUpdateApp.forRole(allRole, roleType);
                    }
                    if (val != 'overallUser') {
                        $("input:checkbox[value='" + $.wmsUserAccreditInfoUpdateApp.purchaseRole.id + "']").attr('checked', 'true');
                    }
                    $('input[name="roleNames"]').on('change', function () {
                        var purchaseRole = $.wmsUserAccreditInfoUpdateApp.purchaseRole;
                        // console.log(purchaseRole);
        ...
        fileForm: function (id) {
            ...
            aContent.success = function (result, textStatus) {
                ...
                if (result.appcode != 1) {
                    ...
                    // $.wmsUserAccreditInfoUpdateApp.oldName = userDate.name;
                    if (userDate.userType == "overallUser") {
                        $.wmsUserAccreditInfoUpdateApp.queryRoleListLoad("wholeJurisdiction", userDate.roleNames);
                    } else if (userDate.userType == "channelUser") {
                        $.wmsUserAccreditInfoUpdateApp.queryRoleListLoad("channelJurisdiction", userDate.roleNames);
                    } else {
                        $.wmsUserAccreditInfoUpdateApp.queryRoleListLoad("", userDate.roleNames);
                    }
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    if (roleType == "") {
                        //混合用户：将渠道和全局用户分类
                        $("#selectRole").append("<div style='margin: 10px 0'>渠道用户：</div>");
                        $.wmsUserAccreditInfoUpdateApp.forRoleLoad(allRole, "channelJurisdiction", roleArray);
                        $("#selectRole").append("<div style='margin: 10px 0'>全局用户：</div>");
                        $.wmsUserAccreditInfoUpdateApp.forRoleLoad(allRole, "wholeJurisdiction", roleArray);
                    } else {
                        $("#selectRole").append("</br>");
                        $.wmsUserAccreditInfoUpdateApp.forRoleLoad(allRole, roleType, roleArray);
                    }
                    $('input[name="roleNames"]').on('change', function () {
                        var purchaseRole = $.wmsUserAccreditInfoUpdateApp.purchaseRole;
                        // console.log(purchaseRole);
            ...
            for (var i = 0; i < allRole.length; i++) {
                if (allRole[i].roleType == roleType) {
                    ...
                    if (allRole[i].name == "采购组员") {
                        $.wmsUserAccreditInfoUpdateApp.purchaseRole['id'] = allRole[i].id;
                        $.wmsUserAccreditInfoUpdateApp.purchaseRole['name'] = allRole[i].name;
                    }
                    var _html = $(html);
                    $("#selectRole").append(_html);
                    childSize++;
    ...
    $(document).ready(function (e) {
        $.wmsUserAccreditInfoUpdateApp.init();
    });
}(jQuery));
