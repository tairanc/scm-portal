        ...
        init: function () {
            ...
            BUI.use(['bui/form','bui/grid','bui/data', 'bui/tooltip'], function (Form, Grid,Data,Tooltip) {
                    ...
                    if (formData.userType == "") {
                        ...
                        if (form.isValid()) {
                            if ($("input[name='roleNames']:checked").length == 0) {
                                ...
                                $.userAccreditInfoAddApp.save(formData, tip);
                ...
                $("#userType").on("change", function () {
                    $.userAccreditInfoAddApp.roleTypeChange();
                });
        ...
        roleTypeChange: function () {
            ...
            switch (val) {
                ...
                case "mixtureUser" :
                    //混合用户
                    $("#channelTypeDiv").show()
                    $.userAccreditInfoAddApp.queryRoleList("");
                    break;
                case  "overallUser" :
                    $("#channelTypeDiv").hide();
                    $.userAccreditInfoAddApp.queryRoleList("wholeJurisdiction");
                    //全局用户
                    break;
                case "channelUser":
                    //渠道用户
                    $("#channelTypeDiv").show()
                    $.userAccreditInfoAddApp.queryRoleList("channelJurisdiction");
                    break;
            }
        },
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    if (roleType == "") {
                        ...
                        $.userAccreditInfoAddApp.forRole(allRole, "channelJurisdiction");
                        $("#selectRole").append("<div style='margin: 10px 0'>全局用户：</div>");
                        $.userAccreditInfoAddApp.forRole(allRole, "wholeJurisdiction");
                    } else {
                        $("#selectRole").append("</br>");
                        $.userAccreditInfoAddApp.forRole(allRole, roleType);
                    }
