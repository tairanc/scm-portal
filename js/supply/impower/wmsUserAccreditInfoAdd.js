...
$(function () {
        ...
        init: function () {
            ...
            BUI.use(['bui/form','bui/grid','bui/data', 'bui/tooltip'], function (Form, Grid,Data,Tooltip) {
                    ...
                    if (formData.userType == "") {
                        ...
                        if (form.isValid()) {
                            if ($("input[name='roleNames']:checked").length == 0) {
                                ...
                                $.wmsUserAccreditInfoAddApp.save(formData, tip);
                ...
                $("#userType").on("change", function () {
                    $.wmsUserAccreditInfoAddApp.roleTypeChange();
                });
        ...
        roleTypeChange: function () {
            ...
            switch (val) {
                ...
                case "mixtureUser" :
                    //混合用户
                    $("#channelTypeDiv").show()
                    $.wmsUserAccreditInfoAddApp.queryRoleList("");
                    break;
                case  "overallUser" :
                    $("#channelTypeDiv").hide();
                    $.wmsUserAccreditInfoAddApp.queryRoleList("wholeJurisdiction");
                    //全局用户
                    break;
                case "channelUser":
                    //渠道用户
                    $("#channelTypeDiv").show()
                    $.wmsUserAccreditInfoAddApp.queryRoleList("channelJurisdiction");
                    break;
            }
        },
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    if (roleType == "") {
                        ...
                        $.wmsUserAccreditInfoAddApp.forRole(allRole, "channelJurisdiction");
                        $("#selectRole").append("<div style='margin: 10px 0'>全局用户：</div>");
                        $.wmsUserAccreditInfoAddApp.forRole(allRole, "wholeJurisdiction");
                    } else {
                        $("#selectRole").append("</br>");
                        $.wmsUserAccreditInfoAddApp.forRole(allRole, roleType);
                    }
    ...
    $(document).ready(function (e) {
        $.wmsUserAccreditInfoAddApp.init();
    });
}(jQuery));
