...
$(function () {
    ...
    $.supplierUpdateApp = {
        ...
        init: function () {
            ...
            BUI.use(['bui/form', 'bui/tooltip', 'bui/uploader', 'bui/tab', 'bui/mask'], function (Form, Tooltip, Uploader, Tab) {
                ...
                $.supplierUpdateApp._form = form;

                $.supplierUpdateApp.supplierCode = $.getUrlParam("supplierCode");
                $("#supplierCode2").val(this.supplierCode);
                //this.querySupplierNatureList();
                $.supplierUpdateApp.queryCountryList();
                $.supplierUpdateApp.querySupplierInfo();
                $.supplierUpdateApp.getAddedChannels();
                setTimeout(function () {
                    $.supplierUpdateApp.initChannel();
                }, 200);
                $.supplierUpdateApp.initSupplierCategory();
                $.supplierUpdateApp.initSupplierBrand();

                /*var supplierKindCode = $('input[name="supplierKindCode"]:checked').val();
                if (supplierKindCode = "oneAgentSelling") {//一件代发
                    $(".certCls").each(
                        function () {
                            $(this).hide();
                        }
                ...
                $('input[name="supplierKindCode"]').on("change", function () {
                    ...
                    var normal3Cert = $.supplierUpdateApp._form.getChild("normal3Cert");  // 证件信息
                    var threeCertUnion = $.supplierUpdateApp._form.getChild("threeCertUnion");  // 证件信息
                ...
                $("#save_btn").on("click", function () {
                    var form1 = $.supplierUpdateApp._form.getChild("baseInfoDiv");// 基本信息
                    var addressFrom = null;
                    if ($('input[name="supplierTypeCode"]:checked').val() == "internalSupplier") {
                        addressFrom = $.supplierUpdateApp._form.getChild("innerSupplier"); // 国内供应商
                    } else {
                        addressFrom = $.supplierUpdateApp._form.getChild("outerSupplier"); // 海外供应商
                    }
                    ...
                    if ($.supplierUpdateApp.checkFinacialAfterSale() == false) {
                        return false;
                    }
                    ...
                    if($('input[name="supplierTypeCode"]:checked').val() == "internalSupplier"){
                        if($('input[name="supplierKindCode"]:checked').val() == "purchase"){
                            if(!$.supplierUpdateApp.checkdate()){
                                return false;
                            }
                            if ($.supplierUpdateApp.checkCertificatePic() == false) {
                                return false;
                            }
                        }
                    }
                  if($.supplierUpdateApp.checkdate()) {
                    $.supplierUpdateApp.save(form.toObject());
                  }
                    /*if (form1.isValid() && form1.isValid()) {

                        if ($('input[name="supplierKindCode"]:checked').val() == "purchase") { // 采购
                            if (addressFrom.isValid() && threeCertUnion.isValid()|| normal3Cert.isValid() && $.supplierUpdateApp.checkdate()) {
                                if ($.supplierUpdateApp.checkCertificatePic()) { // 证件照信息
                                    $.supplierUpdateApp.save(form.toObject());
                                }
                            }
                        } else { // 一键代发
                ...
                //
                $('#supplierTypeCode').on("change", function () {
                    var val = $(this).val();
                    if ("internalSupplier" == val) {
                        $("#certificateTypeId").attr("checked", "checked");
                        $("#certificateTypeId2").removeAttr("checked");
                    }
                });
                ...
                $("#supplierName").on("blur", function () {
                    if(form.isValid()){
                        ...
                        if (supplierName == "" || supplierName == $.supplierUpdateApp.supplierName) {
                            return false;
                        }
                        ...
                        aContent.success = function (result, textStatus) {
                            if (result.appcode != 1) {
                                ...
                                if ("" != result.result && null != result.result && [] != result.result) {
                                    ...
                                    $("#supplierName").val($.supplierUpdateApp.supplierName);
                                }
                            }
                        };
                        $.ajax(aContent);
        ...
        querySupplierInfo: function () {
            var aContent = $.AjaxContent();
            aContent.url = $.scmServerUrl + "supplier/supplier/" + $.supplierUpdateApp.supplierCode;
            aContent.data = {};
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    // $.supplierUpdateApp._form.setRecord(supplierExt);
                    ...
                    $.supplierUpdateApp.supplierName = supplier['supplierName'];
                    $.supplierUpdateApp._channels = supplierChannel;
                    $.supplierUpdateApp._province = supplier['province'];
                    $.supplierUpdateApp._city = supplier['city'];
                    $.supplierUpdateApp._area = supplier['area'];
                    if (supplier['supplierTypeCode'] == "internalSupplier") {
                        $.supplierUpdateApp.setCertificate(supplier['certificateTypeId'], certificate);
                    }
                    $.supplierUpdateApp.setSupplierBase(supplier);
                    $.supplierUpdateApp.setFinancial(financial);
                    $.supplierUpdateApp.setAfterSale(afterSale);
                    $.supplierUpdateApp.hideDiv(supplier['supplierTypeCode'], supplier['certificateTypeId']);

                    //初始化上传控件
                    $.supplierUpdateApp.initPicUpload();
                }
            };
            $.ajax(aContent);
        },
        ...
        hideDiv: function (supplierTypeCode, certificateTypeId) {
            if (supplierTypeCode == "internalSupplier") {
                $("#innerSupplier").show();
                $("#outerSupplier").hide();
                $("#supplierTypeCode").attr("checked", "checked");
                $("#supplierTypeCode2").removeAttr("checked");
                if (certificateTypeId == "normalThreeCertificate") {
                    ...
                    $("#threeCertUnion").show();
                    $("#normal3Cert").hide();
                    $("#certificateTypeId2").attr("checked", "checked");
                    $("#certificateTypeId").removeAttr("checked");
                }
            } else {
                $("#innerSupplier").hide();
                $("#outerSupplier").show();
                $("#supplierTypeCode2").attr("checked", "checked");
                $("#supplierTypeCode").removeAttr("checked");
                $("#certificateTypeId").attr("checked", "checked");
                $("#certificateTypeId2").removeAttr("checked");
                $("#threeCertUnion").hide();
                $("#normal3Cert").hide();
        ...
        setSupplierBase: function (supplier) {
            ...
            for (var key in supplier) {
                $.supplierUpdateApp._form.setFieldValue( key, supplier[key] );
            }
            ...
            if (supplier['supplierKindCode'] == "oneAgentSelling") {//一件代发
                ...
                $("#supplierKindCode2").attr("checked", "checked");
                $("#supplierKindCode").removeAttr("checked");
                ...
                $("#supplierKindCode").attr("checked", "checked");
                $("#supplierKindCode2").removeAttr("checked");
            }
            ...
            $.supplierUpdateApp._isValid = supplier['isValid'];
            $.supplierUpdateApp._supplierKindCode = supplier['supplierKindCode'];
            $.supplierUpdateApp._certificateTypeId = supplier['certificateTypeId'];
            $.supplierUpdateApp.setSupplierType(supplier['supplierTypeCode'], supplier['certificateTypeId']);
            ...
            setTimeout(function () {
                ...
                $.supplierUpdateApp._form.setFieldValue( "country", supplier['country']);
                $.supplierUpdateApp._form.clearErrors();
                $("#id").val(supplier['id']);
            }, 200);
            $("#isValid").val("1");
            $("#isValid2").val("0");
            if ($.supplierUpdateApp._isValid == "1") {
                $("#isValid").attr("checked", "checked");
                $("#isValid2").removeAttr("checked");
            } else {
        ...
        setCertificate: function (certificateTypeId, certificate) {
            ...
            for (var key in certificate) {
                $.supplierUpdateApp._form.setFieldValue( key, certificate[key] );
            }
        ...
        setFinancial: function (financial) {
            ...
            for (var key in financial) {
                $.supplierUpdateApp._form.setFieldValue( key, financial[key] );
            }
        },
        /**
         * 设置售后信息
        ...
        setAfterSale: function (afterSale) {
            ...
            for (var key in afterSale) {
                $.supplierUpdateApp._form.setFieldValue( key, afterSale[key] );
            }
        ...
        initChannel: function () {
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    setTimeout(function () {
                        $.supplierUpdateApp.createChannelCheckBox(channels);
                    }, 100);
                }
            };
            $.ajax(aContent);
        ...
        createChannelCheckBox: function (channels) {
            var addedChannels = $.supplierUpdateApp._currentChannels;
            for (var i = 0; i < channels.length; i++) {
                ...
                var label = $('<label class="control-label checkbox" style="text-align: left; margin-left: 10px; width: auto;"></label>');
                var ck = $('<input name="channel" type="checkbox" value="' + tmp + '" disabled/>');
                if (flag) {
        ...
        getAddedChannels: function () {
            ...
            aContent.data = {supplierCode: $.supplierUpdateApp.supplierCode};
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    BUI.Message.Alert(result.databuffer, 'warning');
                } else {
                    $.supplierUpdateApp._currentChannels = result.result;
                }
            };
            $.ajax(aContent);
        },
        ...
        initSupplierCategory: function () {
            ...
            BUI.use(['bui/grid', 'bui/data', 'bui/overlay', 'bui/mask'], function (Grid, Data, Overlay) {
                ...
                $.supplierUpdateApp.store = new Store({
                    ...
                    autoLoad: true,
                    params: {supplierCode: $.supplierUpdateApp.supplierCode}
                });
                ...
                grid.on("cellclick", function (ev) {
                    ...
                    if (ev.domTarget.className == "grid-command btn-del") {
                        var flag = false;
                        //判断是否又该分类相关的品牌
                        var categoryBrands = $.supplierUpdateApp.brandTable.getData();
                        if (categoryBrands.length > 0) {
                            var msg = "分类[" + record['categoryName'] + "]";
                            for (var i = 0; i < categoryBrands.length; i++) {
                                ...
                                if (record['categoryId'] == categoryBrands[i]['categoryId'] && status != "3") {
                                    ...
                                    return false;
                                    //删除分类
                                    $.supplierUpdateApp.store.remove(record);
                                }
                            }
                            if (!flag) {
                                //删除分类
                ...
                var dialog = new Overlay.Dialog({
                    ...
                    loader: {
                        ...
                        callback: function (text) {
                            ...
                            var currentCategorys = $.supplierUpdateApp.store.getResult();
                            var _categoryIds = "";
                ...
                function addSupplierCategory(categorys) {
                    var results = $.supplierUpdateApp.store.getResult();
                    var msg = "";
                    for (var i = 0; i < categorys.length; i++) {
                        ...
                        if (checkResult[0] == true) {
                            ...
                            //已经添加过的不添加
                            if (!flag) {
                                $.supplierUpdateApp.store.add(record);
                            }
                        } else {
        ...
        handlerSelectedBrand: function () {
            var items = $.supplierUpdateApp.brandTable.getData();
            var brandCodes = [];
        ...
        initSupplierBrand: function () {
            BUI.use(['bui/form', 'bui/overlay', 'bui/mask'], function (Form, Overlay) {
                ...
                $.supplierUpdateApp.brandTable = new DynamicGrid(columns);
                var dataConfig = {
                    gridId: "brandGrid",
                    uploadUrl: $.scmServerUrl + 'qinniu/upload/supply',
                    uploadMax: 3,
                    dataUrl: $.scmServerUrl + "supplier/supplierBrands?supplierCode=" + $.supplierUpdateApp.supplierCode,
                    batchDownloadUrl: $.scmServerUrl + 'qinniu/urls?thumbnail=1',
                    // sortName:""
                };

                $.supplierUpdateApp.brandTable.initDataAjax({}, dataConfig,function () {
                    var items = $.supplierUpdateApp.brandTable.getData();
                    var brandCodes = [];
                    for(var i=0;i<items.length;i++){
                        brandCodes.push(items[i].brandCode);
                    }
                ...
                var dialog = new Overlay.Dialog({
                    ...
                    buttons: [
                        {
                            ...
                            handler: function () {
                                ...
                                if (data.length > 0) {
                                    $.supplierUpdateApp.brandTable.addRow(data);

                                    $.supplierUpdateApp.handlerSelectedBrand();
                    ...
                    loader: {
                        ...
                        autoLoad: false, //自动加载
                        callback: function (text) {
                            var categoryId = $.supplierUpdateApp.getCategoryId();
                ...
                $("#add_brand_btn").on('click', function () {
                    var _categroys = $.supplierUpdateApp.store.getResult();
                ...
                function addSupplierBrand(categoryBrands) {
                    var _categroys = $.supplierUpdateApp.store.getResult();
                    var results = $.supplierUpdateApp.brandTable.getData();
                    var data = [];
                    var msg = "";
                    for (var i = 0; i < categoryBrands.length; i++) {
                        var categoryName = "";
        ...
        getCategoryId: function () {
            var supplierCategory = $.supplierUpdateApp.store.getResult();
            var categoryId = "";
        ...
        checkdate: function () {
            ...
            if (supplierTypeCode == "internalSupplier") {//国内供应商
                if (certificateTypeId == "normalThreeCertificate") {//普通三证
                    ...
                    if (businessLicenceStartDate > businessLicenceEndDate) {
                        BUI.Message.Alert("营业执照结束日期不能小于起始日期", "warning");
                        return false;
                    }else if ((!businessLicenceStartDate && businessLicenceEndDate )|| (businessLicenceStartDate&&!businessLicenceEndDate)) {
                    ...
                    if (organRegistraStartDate > organRegistraEndDate) {
                        BUI.Message.Alert("组织机构代码证结束日期不能小于起始日期", "warning");
                        return false;
                    }else if ((!organRegistraStartDate&&organRegistraEndDate) || (organRegistraStartDate&&!organRegistraEndDate)) {
                    ...
                    if (taxRegistrationStartDate > taxRegistrationEndDate) {
                        BUI.Message.Alert("税务登记证结束日期不能小于起始日期", "warning");
                        return false;
                    }else if ((!taxRegistrationStartDate&&taxRegistrationEndDate) || (taxRegistrationStartDate&&!taxRegistrationEndDate)) {
                      BUI.Message.Alert("请补全税务登记证有效期", "warning");
                      return false;
                    }
                    var idCardStartDate = $('#idCardStartDate').val();
                    var idCardEndDate = $('#idCardEndDate').val();
                    if (idCardStartDate > idCardEndDate) {
                        BUI.Message.Alert("法人身份证结束日期不能小于起始日期", "warning");
                        return false;
                    }else if ((!idCardStartDate&&idCardEndDate )|| (idCardStartDate&&!idCardEndDate)) {
                    ...
                    if (multiCertificateStartDate > multiCertificateEndDate) {
                        BUI.Message.Alert("多证合一结束日期不能小于起始日期", "warning");
                        return false;
                    }else if ((!multiCertificateStartDate&&multiCertificateEndDate) || (multiCertificateStartDate&&!multiCertificateEndDate)) {
                      BUI.Message.Alert("请补全多证合一有效期", "warning");
                      return false;
                    }
                    var idCardStartDate2 = $('#idCardStartDate2').val();
                    var idCardEndDate2 = $('#idCardEndDate2').val();
                    if (idCardStartDate2 > idCardEndDate2) {
                        BUI.Message.Alert("法人身份证结束日期不能小于起始日期", "warning");
                        return false;
                    }else if ((!idCardStartDate2&&idCardEndDate2) || (idCardStartDate2&&!idCardEndDate2)) {
                      BUI.Message.Alert("请补全法人身份证有效期", "warning");
                      return false;
                    }
                }
        ...
        checkFinacialAfterSale: function () {
            var myform = $.supplierUpdateApp._form.getChild("supplierFinancial");
            myform.valid()
            if (myform.isValid() == false) {
                $.supplierUpdateApp.tab.setSelected($.supplierUpdateApp.tab.getItemAt(2));
                return false;
            }
            var myform2 = $.supplierUpdateApp._form.getChild("supplierAfterSale")
            myform2.valid()
            if (myform2.isValid() == false) {
                $.supplierUpdateApp.tab.setSelected($.supplierUpdateApp.tab.getItemAt(3));
                return false;
            }
            return true;
        },
        ...
        save: function (fromData, tip) {
            ...
            //删除图片
            if ($.supplierUpdateApp.uploadExt1.removeFiles() &&
                $.supplierUpdateApp.uploadExt2.removeFiles() &&
                $.supplierUpdateApp.uploadExt3.removeFiles() &&
                $.supplierUpdateApp.uploadExt4.removeFiles() &&
                $.supplierUpdateApp.uploadExt5.removeFiles() &&
                $.supplierUpdateApp.uploadExt6.removeFiles() &&
                $.supplierUpdateApp.uploadExt7.removeFiles() &&
                $.supplierUpdateApp.uploadExt8.removeFiles()) {
            } else {
                return false;
            }
            ...
            if ($.checkUploadStatus() == true) {
                ...
                //代理品牌数据处理
                var supplierBrand = $.supplierUpdateApp.brandTable.getData();
                if (supplierBrand.length == 0) {
                    BUI.Message.Alert("供应商代理品牌不能为空", "warning");
                    return false;
                }else{
                    var _flag = false;
                    for(var i=0; i<supplierBrand.length; i++){
                        if(supplierBrand[i]['status'] != "3"){
                            _flag = true;
                            break;
                        }
                    }
                ...
                var supplierKindCode = $('input[name="supplierKindCode"]:checked').val();
                //供应商性质
                fromData['supplierKindCode'] = supplierKindCode;
                //供应商相关渠道数据处理
                ...
                if ($.supplierUpdateApp.brandTable.isValid()) {
                    $.supplierUpdateApp.setSpecialField(fromData);
                    fromData['channel'] = channel;
                    var id = $("#id").val();
                    var aContent = $.AjaxContent();
                    aContent.url = $.scmServerUrl + "supplier/supplier/" + id;
    ...
    $(document).ready(function (e) {
        $.supplierUpdateApp.init();
    });
    //页面完全加载完成后设置单选框默认选中
    window.onload = function () {
        $.setRadioChecked('isValid', $.supplierUpdateApp._isValid);
    }
}(jQuery));
