...
$(function () {
    $.supplierApplyAddDetailApp = {
        _submitFlag : "1",//提交标志,默认为1-可以提交,0-不能提交
        _errorMsg : null,//错误信息
        supplierCode: '',
        supplierName: '',
        _isValid: '',//是否启用
        ...
        init: function (supplierCode) {
            ...
            $.supplierApplyAddDetailApp.queryCountryList();
        ...
        initSupplierBrand: function () {
            BUI.use(['bui/form', 'bui/overlay', 'bui/mask'], function (Form, Overlay) {
                ...
                $.supplierApplyAddDetailApp.brandTable = new DynamicGrid(columns);
                var dataConfig = {
                    gridId: "brandGrid",
                    uploadUrl: $.scmServerUrl + 'qinniu/upload/supply',
                    uploadMax: 0,
                    dataUrl: $.scmServerUrl + "supplier/supplierBrands?supplierCode=" + $.supplierApplyAddDetailApp.supplierCode,
                    batchDownloadUrl: $.scmServerUrl + 'qinniu/urls?thumbnail=1',
                    // sortName:""
                };
                $.supplierApplyAddDetailApp.brandTable.initDataAjax({}, dataConfig);
    ...
    $.supplierApplyAddApp = {
        ...
        init: function () {
            $.supplierApplyAddDetailApp.queryCountryList();
            BUI.use(['bui/form', 'bui/overlay', 'bui/tab'], function (Form, Overlay, Tab) {
                ...
                $.supplierApplyAddApp.dialog = new Overlay.Dialog({
                    title: '选择供应商',
                    width: 1200,
                    elCls:'dialogTop10',
                    mask: true,
                ...
                $("#selectSupplier").on("click", function () {
                    //$.supplierApplyAddApp.dialog.render();
                    $.supplierApplyAddApp.dialog.get('loader').load({});
                    $.supplierApplyAddApp.dialog.show();  
                    console.log($(".bui-dialog"))
                    $(".bui-dialog").css("height","700px;");
                    $(".bui-dialog")[0].style.top = '50px';
                    $(".bui-stdmod-body").css("overflow-y","auto")
                    $(".bui-stdmod-body").css("overflow-x","hidden")
                    $(".bui-stdmod-body").css("height","634px");                 
                });
                $("#save_btn").on("click", function () {
                    ...
                    if (form.isValid()) {
                        ...
                        if (rel) {
                            $.supplierApplyAddDetailApp.save(formData);
                        }
                    }
                });
                ...
                $("#commit_btn").on("click", function () {
                    if($.supplierApplyAddDetailApp._submitFlag == "0"){
                        BUI.Message.Alert($.supplierApplyAddDetailApp._errorMsg, "warning");
                        return false;
                    }
                    if ($.supplierApplyAddDetailApp.supplierCode) {
