...
$(function () {
    ...
    $.logisticsUpdateApp = {
        ...
        init: function () {
            ...
            $.logisticsUpdateApp.queryIsValid(id);
            ...
            BUI.use(['bui/form', 'bui/grid','bui/data','bui/tooltip'], function (Form, Grid,Data,Tooltip) {
                ...
                grid.on("aftershow", function() {
                    $.logisticsUpdateApp.fileForm(id);
                })
                grid.render();
                    ...
                    if (formData.userType != "overallUser" && formData.channelCode == "") {
                        ...
                        if (form.isValid()) {
                            if ($("input[name='roleNames']:checked").length == 0) {
                                ...
                                $.logisticsUpdateApp.updateUser(formData, tip);
                            }
                        }
                    }
                });
                ...
                $("#userType").on("change", function () {
                    ...
                    // var purchaseRole = $.logisticsUpdateApp.purchaseRole;
                    ...
                    switch (val) {
                        ...
                        case "mixtureUser" :
                            //混合用户
                            $("#channelTypeDiv").show();
                            $.logisticsUpdateApp.queryRoleList("",val);
                            break;
                        case  "overallUser" :
                            $("#channelTypeDiv").hide();
                            $.logisticsUpdateApp.queryRoleList("wholeJurisdiction",val);
                            //全局用户
                            break;
                        case "channelUser":
                            //渠道用户
                            $("#channelTypeDiv").show();
                            $.logisticsUpdateApp.queryRoleList("channelJurisdiction",val);
                            break;
                    }
                ...
                //             //传到后台做校验
                //             $.logisticsUpdateApp.checkPurchase();
                //         }
                //     }
                // });
            });
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    if (roleType == "") {
                        ...
                        $.logisticsUpdateApp.forRole(allRole, "channelJurisdiction");
                        $("#selectRole").append("<div style='margin: 10px 0'>全局用户：</div>");
                        $.logisticsUpdateApp.forRole(allRole, "wholeJurisdiction");
                    } else {
                        $.logisticsUpdateApp.forRole(allRole, roleType);
                    }
                    if (val != 'overallUser') {
                        $("input:checkbox[value='" + $.logisticsUpdateApp.purchaseRole.id + "']").attr('checked', 'true');
                    }
                    $('input[name="roleNames"]').on('change', function () {
                        var purchaseRole = $.logisticsUpdateApp.purchaseRole;
                        // console.log(purchaseRole);
        ...
        fileForm: function (id) {
            ...
            aContent.success = function (result, textStatus) {
                ...
                if (result.appcode != 1) {
                    ...
                    // $.logisticsUpdateApp.oldName = userDate.name;
                    if (userDate.userType == "overallUser") {
                        $.logisticsUpdateApp.queryRoleListLoad("wholeJurisdiction", userDate.roleNames);
                    } else if (userDate.userType == "channelUser") {
                        $.logisticsUpdateApp.queryRoleListLoad("channelJurisdiction", userDate.roleNames);
                    } else {
                        $.logisticsUpdateApp.queryRoleListLoad("", userDate.roleNames);
                    }
            ...
            aContent.success = function (result, textStatus) {
                if (result.appcode != 1) {
                    ...
                    if (roleType == "") {
                        //混合用户：将渠道和全局用户分类
                        $("#selectRole").append("<div style='margin: 10px 0'>渠道用户：</div>");
                        $.logisticsUpdateApp.forRoleLoad(allRole, "channelJurisdiction", roleArray);
                        $("#selectRole").append("<div style='margin: 10px 0'>全局用户：</div>");
                        $.logisticsUpdateApp.forRoleLoad(allRole, "wholeJurisdiction", roleArray);
                    } else {
                        $("#selectRole").append("</br>");
                        $.logisticsUpdateApp.forRoleLoad(allRole, roleType, roleArray);
                    }
                    $('input[name="roleNames"]').on('change', function () {
                        var purchaseRole = $.logisticsUpdateApp.purchaseRole;
                        // console.log(purchaseRole);
            ...
            for (var i = 0; i < allRole.length; i++) {
                if (allRole[i].roleType == roleType) {
                    ...
                    if (allRole[i].name == "采购组员") {
                        $.logisticsUpdateApp.purchaseRole['id'] = allRole[i].id;
                        $.logisticsUpdateApp.purchaseRole['name'] = allRole[i].name;
                    }
                    var _html = $(html);
                    $("#selectRole").append(_html);
                    childSize++;
