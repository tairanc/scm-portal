...
$(function(){
    ...
    $.storehouseWarehouseNoticeInfoApp = {
        ...
        init:function(){
            ...
            $.storehouseWarehouseNoticeInfoApp.filePurchaseOrderForm(purchaseOrderCode); //填充采购单部分
        	...
        	BUI.use(['bui/form','bui/grid','bui/tooltip', 'bui/data'],function(Form,Grid,Tooltip, Data){
                ...
                var Store = Data.Store,
                    columns = [
                        ...
                        {title: '货号', dataIndex: 'itemNo', width: "150px", elCls: 'center',renderer:function(value){
                            return value?value:"-";
                        }},
                        ...
                        {title: '批次号', dataIndex: 'batchCode', width: "150px", elCls: 'center',renderer:function(value){
                            return value?value:"-";
                        }},
                        {title: '生产编码', dataIndex: 'produceCode', width: "150px", elCls: 'center',renderer:function(value){
                            return value?value:"-";
                        }},
                        {title: '生产日期', dataIndex: 'productDate', width: "150px", elCls: 'center',renderer:function(value){
                            return value?value:"-";
                        }},
                        {title: '截止保质日期', dataIndex: 'expireDate', width: "150px", elCls: 'center',renderer:function(value){
                            return value?value:"-";
                        }},
                        {title: '理论保质期限(天)', dataIndex: 'shelfLifeDays', width: "150px", elCls: 'center',renderer:function(value){
                            return value?value:"-";
                        }}
                    ]
        ...
        filePurchaseOrderForm:function (purchaseOrderCode) {
            ...
            aContent.success = function(result,textStatus){
                if(result.appcode != 1){
                    ...
                    if(purcharseDtl.currencyType == $.storehouseWarehouseNoticeInfoApp.dollar){
                        $("#totalFeeTip").text('美元');
                        $("#purchasePriceD").text('(美元)');
                        $("#totalPurchaseAmountD").text('(美元)');
                    }
                    if(purcharseDtl.currencyType == $.storehouseWarehouseNoticeInfoApp.rmb){
                        $("#totalFeeTip").text('元');
                    ...
                    if(status == $.storehouseWarehouseNoticeInfoApp.FREEZE){
                        $("#statusTip").text("冻结");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.RECEIVE_EXCEPTION){
                        $("#statusTip").text("收货异常");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.RECEIVE_ALL){
                        $("#statusTip").text("全部收货");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.REJECT){//审核驳回的处理
                        $("#statusTip").text("审核驳回");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.CANCEL){ //提交审核处理
                        $("#statusTip").text("作废")
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.PASS){ //审核通过的处理的处理
                        $("#statusTip").text("审核通过");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.AUDIT){ //提交审核处理
                        $("#statusTip").text("提交审核");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.HOLD){ //暂存的处理
                        $("#statusTip").text("暂存");
                    };
                    if(status == $.storehouseWarehouseNoticeInfoApp.WAREHOUSE_NOTICE){ //入库通知
                        $("#statusTip").text("入库通知");
                    };
                };
            };
    ...
    $(document).ready(function(e) {
        $.storehouseWarehouseNoticeInfoApp.init();
    });
}(jQuery));
